...
namespace Windows.Forms.Controls.MyListBox
{
    ...
    public partial class MyListBox : Control
    {
        public MyListBox()
        {
            ...
            this.iconSizeMode = MyListBoxItemIcon.Small;
            this.items = new MyListBoxItemCollection(this);
            ...
            chatVScroll = new MyListBoxVScroll(this);
        }

        #region Properties

        private MyListBoxItemIcon iconSizeMode;
        /// <summary>
        /// 获取或者设置列表的图标模式
        /// </summary>
        ...
        private MyListBoxItemCollection items;
        /// <summary>
        /// 获取列表中所有列表项的集合
        /// </summary>
        ...
        public MyListBoxItemCollection Items {
            get {
                if (items == null)
                    items = new MyListBoxItemCollection(this);
                return items;
            }
        }
        ...
        private MyListBoxVScroll chatVScroll;    //滚动条

        private MyListBoxItem m_mouseOnItem;
        private bool m_bOnMouseEnterHeaded;     //确定用户绑定事件是否被触发
        private MyListBoxSubItem m_mouseOnSubItem;
                                ...
                                if (m_mouseOnSubItem != null) {             //如果当前鼠标下子项不为空
                                    if (items[i].SubItems[j].HeadRect.Contains(m_ptMousePos)) {     //判断鼠标是否在头像内
                                        if (!m_bOnMouseEnterHeaded) {       //如果没有触发进入事件 那么触发用户绑定的事件
                                            OnMouseEnterHead(new MyListBoxtEventArgs(this.m_mouseOnSubItem, this.selectSubItem));
                                            m_bOnMouseEnterHeaded = true;
                                        }
                                    } else {
                                        if (m_bOnMouseEnterHeaded) {        //如果已经执行过进入事件 那触发用户绑定的离开事件
                                            OnMouseLeaveHead(new MyListBoxtEventArgs(this.m_mouseOnSubItem, this.selectSubItem));
        ...
        protected override void OnClick(EventArgs e) {
            ...
            foreach (MyListBoxItem item in items) {
                if (item.Bounds.Contains(m_ptMousePos)) {
                    if (item.IsOpen) {
                        foreach (MyListBoxSubItem subItem in item.SubItems) {
        ...
        protected override void OnDoubleClick(EventArgs e) {
            ...
            if (this.selectSubItem != null)     //如果选中项不为空 那么触发用户绑定的双击事件
                OnDoubleClickSubItem(new MyListBoxtEventArgs(this.m_mouseOnSubItem, this.selectSubItem));
        ...
        protected virtual void DrawItem(Graphics g, MyListBoxItem item, Rectangle rectItem, SolidBrush sb) {
            ...
            g.DrawString("[" + item.SubItems.Count + "]", this.Font, sb,
        ...
        protected virtual void DrawSubItem(Graphics g, MyListBoxSubItem subItem, ref Rectangle rectSubItem, SolidBrush sb) {
            if (subItem.Equals(selectSubItem)) {        //判断改子项是否被选中
                rectSubItem.Height = (int)MyListBoxItemIcon.Large;
            ...
            if (iconSizeMode == MyListBoxItemIcon.Large)         //没有选中则根据 图标模式绘制
        ...
        protected virtual void DrawHeadImage(Graphics g, MyListBoxSubItem subItem, Rectangle rectSubItem) {
            ...
            if (subItem.Status == MyListBoxSubItem.UserStatus.OffLine)
                g.DrawImage(subItem.GetDarkImage(), subItem.HeadRect);
            else {
                g.DrawImage(subItem.HeadImage, subItem.HeadRect);       //如果在线根据在想状态绘制小图标
                //if (subItem.Status == MyListBoxSubItem.UserStatus.QMe)
                //    g.DrawImage(Resources.QMe, new Rectangle(subItem.HeadRect.Right - 10, subItem.HeadRect.Bottom - 10, 11, 11));
                //if (subItem.Status == MyListBoxSubItem.UserStatus.Away)
                //    g.DrawImage(Resources.Away, new Rectangle(subItem.HeadRect.Right - 10, subItem.HeadRect.Bottom - 10, 11, 11));
                //if (subItem.Status == MyListBoxSubItem.UserStatus.Busy)
                //    g.DrawImage(Resources.Busy, new Rectangle(subItem.HeadRect.Right - 10, subItem.HeadRect.Bottom - 10, 11, 11));
                //if (subItem.Status == MyListBoxSubItem.UserStatus.DontDisturb)
                //    g.DrawImage(Resources.Dont_Disturb, new Rectangle(subItem.HeadRect.Right - 10, subItem.HeadRect.Bottom - 10, 11, 11));
            }
        ...
        protected virtual void DrawLargeSubItem(Graphics g, MyListBoxSubItem subItem, Rectangle rectSubItem) {
            rectSubItem.Height = (int)MyListBoxItemIcon.Large;       //重新赋值一个高度
        ...
        protected virtual void DrawSmallSubItem(Graphics g, MyListBoxSubItem subItem, Rectangle rectSubItem) {
            rectSubItem.Height = (int)MyListBoxItemIcon.Small;               //重新赋值一个高度
            StringFormat sf = new StringFormat();
            sf.LineAlignment = StringAlignment.Center;
        ...
        public MyListBoxSubItem[] GetSubItemsById(int userId) {
            List<MyListBoxSubItem> subItems = new List<MyListBoxSubItem>();
            for (int i = 0, lenI = this.items.Count; i < lenI; i++) {
                for (int j = 0, lenJ = items[i].SubItems.Count; j < lenJ; j++) {
        ...
        public class MyListBoxItemCollection : IList, ICollection, IEnumerable
        {
            ...
            private MyListBoxItem[] m_arrItem;
            private MyListBox owner;  //所属的控件

            public MyListBoxItemCollection(MyListBox owner) { this.owner = owner; }
            //确认存储空间
            private void EnsureSpace(int elements) {
                if (m_arrItem == null)
                    m_arrItem = new MyListBoxItem[Math.Max(elements, 4)];
                else if (this.count + elements > m_arrItem.Length) {
                    MyListBoxItem[] arrTemp = new MyListBoxItem[Math.Max(this.count + elements, m_arrItem.Length * 2)];
            ...
            public int IndexOf(MyListBoxItem item) {
                return Array.IndexOf<MyListBoxItem>(m_arrItem, item);
            }
            /// <summary>
            /// 添加一个列表项
            /// </summary>
            ...
            int IList.Add(object value) {
                if (!(value is MyListBoxItem))
                    throw new ArgumentException("Value cannot convert to ListItem");
                this.Add((MyListBoxItem)value);
                return this.IndexOf((MyListBoxItem)value);
            }
            ...
            bool IList.Contains(object value) {
                if (!(value is MyListBoxItem))
                    throw new ArgumentException("Value cannot convert to ListItem");
                return this.Contains((MyListBoxItem)value);
            }
            ...
            void IList.Insert(int index, object value) {
                if (!(value is MyListBoxItem))
                    throw new ArgumentException("Value cannot convert to ListItem");
                this.Insert(index, (MyListBoxItem)value);
            }
            ...
            void IList.Remove(object value) {
                if (!(value is MyListBoxItem))
                    throw new ArgumentException("Value cannot convert to ListItem");
                this.Remove((MyListBoxItem)value);
            }
            ...
            object IList.this[int index] {
                ...
                set {
                    if (!(value is MyListBoxItem))
                        throw new ArgumentException("Value cannot convert to ListItem");
                    this[index] = (MyListBoxItem)value;
                }
            }
