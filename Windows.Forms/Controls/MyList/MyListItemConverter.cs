...
namespace Windows.Forms.Controls.MyList
{
    ...
    internal class MyListItemConverter : ExpandableObjectConverter
    {
        ...
        public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture,
            ...
            if (destinationType == typeof(InstanceDescriptor) && (value is MyListItem)) {
                ConstructorInfo constructor = null;
                MyListItem item = (MyListItem)value;
                MyListSubItem[] subItems = null;
                //MessageBox.Show("Convertto Start Item:" + item.Text);
                //MessageBox.Show("Item.SubItems.Count:" + item.SubItems.Count);
                if (item.SubItems.Count > 0) {
                    subItems = new MyListSubItem[item.SubItems.Count];
                    item.SubItems.CopyTo(subItems, 0);
                }
                //MessageBox.Show("Item.SubItems.Count:" + item.SubItems.Count);
                if (item.Text != null && subItems != null)
                    constructor = typeof(MyListItem).GetConstructor(new Type[] { typeof(string), typeof(MyListSubItem[]) });
                ...
                if (subItems != null)
                    constructor = typeof(MyListItem).GetConstructor(new Type[] { typeof(MyListSubItem[]) });
                if (constructor != null)
                    return new InstanceDescriptor(constructor, new object[] { subItems }, false);
                if (item.Text != null) {
                    //MessageBox.Show("StartGetConstructor(text)");
                    constructor = typeof(MyListItem).GetConstructor(new Type[] { typeof(string), typeof(bool) });
                }
                //MessageBox.Show("Constructor(Text):" + (constructor != null));
                if (constructor != null) {
                    //System.Windows.Forms.MessageBox.Show("text OK");
                    return new InstanceDescriptor(constructor, new object[] { item.Text, true });
                }
            }
